...
static u16 colors[64],arrow_glyph;
...
static void palette(u16 bank,const u16 *p) {
  for(u16 i=0;i<16;++i)colors[bank*16+i]=p[i];
...
static void upload_actor(u16 slot,u16 f) {
  const u16 *p=(const u16 *)(WORD+0x20000UL+slot*0x4100UL+48+f*4096UL);
...
static bool play_stream(u16 id,u16 channel,bool loop) {
  ...
  if(channel)++trace->voice_count;else ++trace->bgm_count;
  return true;
}
...
static void execute(void) {
  ...
  case 1: {
    ...
    if(error) {mode=HALT;return;}palette(0,p);
    if(flags&16)fade(aux,true);
    break;
  }
  case 2: {
    if(slot>2) {error=MCD_ERR_ARGUMENT;mode=HALT;return;}
    ...
    if(a->asset!=target) {
      ...
      /* Each 0x4100-byte cache slot includes its 48-byte metadata. */
      u16 *dest=(u16 *)(WORD+0x20000UL+slot*0x4100UL);const u16 *src=(const u16 *)SCRATCH;
      for(u16 i=0;i<8216;++i)dest[i]=src[i];
      a->asset=target;
  ...
  case 5: {
    if(flags&2) {
      if(aux==1)io_done(MCD_stopCDDA());
      else {if(!io_done(MCD_stopStream(0)))return;io_done(MCD_stopStream(1));}
    } else if(target>=0) {
      const u8 *r=resource(target);u16 kind=be16(r+8);
      if(kind==5)io_done(MCD_playCDDA(be16(r+10),count!=0));
...
void NOVEL_run(void) {
  ...
  for(u16 i=0;i<3;++i)actors[i].asset=-1;
